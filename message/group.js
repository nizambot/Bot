...
module.exports = welcome = async (dha, anu) => {
		...
		try {
            ...
            dha.sendMessage(anu.jid, 'Halo! Terima Kasih sudah Mengundangku, Jika ingin Menggunakan Bot Ketik .menu', 'conversation')
            }
             if (anu.action == 'add' && !mem.includes(dha.user.jid)) {
            ...
            if (!welkom.includes(anu.jid)) return
                ...
                buttons = [{buttonId: `#alquran 1`,buttonText:{displayText: 'AL FATIHAH'},type:1}]
                imageMsg = (await dha.prepareMessageMedia((buff), 'imageMessage', {thumbnail: buff})).imageMessage
                buttonsMessage = { contentText: `${out}`, footerText: 'MARI KITA DOA KAN BERSAMA BUAT YG KLUAR', imageMessage: imageMsg, buttons: buttons, headerType: 4 }
                prep = await dha.prepareMessageFromContent(mdata.id,{buttonsMessage},{})
                dha.relayWAMessage(prep)
            }
		} catch (e) {
